...
function Dashboard({ knowledge, setPage, startFocus, openReading, readingState }) {
  ...
  return <section className="command-home" aria-label="今日成长桌面">
    <header className="command-greeting"><span>PERSONAL GROWTH COMMAND</span><h1>晚上好，Wesley</h1><p>今天，让一个观点真正改变明天。</p></header>
    <div className="command-orbit">
      <motion.button className="command-module action-module glass-surface-static" onClick={startFocus} whileHover={{ y:-7,scale:1.015 }} whileTap={{ scale:.965 }} transition={{ type:"spring",stiffness:310,damping:28 }}><span className="module-kicker"><Lightning size={16} weight="fill"/> 今日行动</span><strong>完成个人知识<br/>检索模块</strong><p>补齐引用回链和 3 个关键测试</p><div><Clock size={16}/>90 分钟 <FileText size={16}/>实现文档</div><em>开始推进 <ArrowRight size={17}/></em></motion.button>
      <motion.button layoutId="reading-module" className="command-module reading-module glass-surface-static" onClick={openReading} whileHover={{ scale:1.022 }} whileTap={{ scale:.975 }} transition={{ type:"spring",stiffness:280,damping:30 }}><span className="reading-halo" aria-hidden="true"/><span className="module-kicker"><BookOpenText size={17} weight="fill"/> 今日精读 · 25 分钟</span><h2>《卓有成效的管理者》</h2><h3>第 2 章：掌握自己的时间</h3><div className="reading-steps">{readingSteps.map((step,index) => <span key={step} className={readingState.steps?.[index] ? "done" : ""}><i>{readingState.steps?.[index] ? <Check size={12}/> : index+1}</i>{step}</span>)}</div><span className="reading-cta">进入沉浸精读 <ArrowRight size={18}/></span></motion.button>
      <motion.button className="command-module knowledge-module glass-surface-static" onClick={() => setPage("library")} whileHover={{ y:-7,scale:1.015 }} whileTap={{ scale:.965 }} transition={{ type:"spring",stiffness:310,damping:28 }}><span className="module-kicker"><Database size={16} weight="fill"/> 知识宇宙</span><strong>{activeKnowledge.length}</strong><p>份知识资产 · {activeKnowledge.filter((item)=>item.status!=="ready").length} 待整理</p><span className="mini-constellation"><Books size={29} weight="duotone"/></span><em>进入知识库 <ArrowRight size={17}/></em></motion.button>
      <motion.button className="command-module review-module glass-surface-static" onClick={() => setPage("review")} whileHover={{ y:-7,scale:1.015 }} whileTap={{ scale:.965 }} transition={{ type:"spring",stiffness:310,damping:28 }}><span className="module-kicker"><ArrowCounterClockwise size={16}/> 成长复盘</span><strong>连续推进 6 天</strong><p>本周形成 3 条成长证据</p><span className="review-pulse"><TrendUp size={25}/></span><em>回看这一周 <ArrowRight size={17}/></em></motion.button>
    </div>
    <button className="reflection-strip glass-surface-static" onClick={openReading}><span><ChatCircle size={21}/><strong>完成后分享读书想法与笔记</strong><small>哪一个观点最值得带进明天？</small></span><em>{readingState.completed ? "继续完善笔记" : "记录并分享"}<ArrowRight size={18}/></em></button>
  </section>;
}
