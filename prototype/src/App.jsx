...
import {
  ...
  TrayArrowDown, TrendUp, UploadSimple, X } from
...
function GlassSurface({ className = "", children, as: Component = motion.section, ...props }) {
  ...
  return <Localize>
    ...
    onPointerLeave={() => setTilt({ rotateX: 0, rotateY: 0 })} {...props}>{children}</Component></Localize>;
...
function MobileNav({ page, setPage }) {
  return <Localize><nav className="mobile-nav glass-surface-static" aria-label="移动端导航">
...
function TopBar({ page, setPage, openUpload, startFocus }) {
  ...
  return <Localize><header className="topbar">
...
function GrowthConstellation({ activeDomain, setActiveDomain, startFocus }) {
  return <Localize><div className="constellation" aria-label="2026 成长主线">
...
function AnswerContent({ answer, onConvert, onSave, compact = false }) {
  return <Localize><>
    ...
    <div className="citation-block"><span className="eyebrow">原始依据 · 点击展开</span>{answer.citations.map((item, index) => <details className="citation-detail" key={`${item.id}-${index}`}><summary><Database size={18} /><span><strong>{item.title}</strong><small>{item.source}</small></span><CaretDown size={16} /></summary><p>{item.content || item.summary || "该文件暂未解析正文。"}</p></details>)}</div>
...
function makeAnswer(query, knowledge, scope) {
  ...
  const tokens = [...new Set([...(normalized.match(/[a-z0-9]+/g) || []), ...(normalized.match(/[\u4e00-\u9fff]{2,}/g) || []).flatMap((part) => Array.from({ length: part.length - 1 }, (_, index) => part.slice(index, index + 2)))])];
  const pool = knowledge.filter((item) => !item.deleted && (scope === "全部知识" || item.space === scope));
  ...
  const matches = scored.filter((item) => item.score > 0.5);const citations = (matches.length ? matches : scored).slice(0, 3);const focus = citations[0];
  const evidence = citations.map((item) => item.summary.replace(/。$/, "")).join("；");
  return { title: citations.length ? `${matches.length ? '相关资料' : '最近资料'}：${focus.title}` : '当前范围暂无资料', body: evidence ? `${matches.length ? '按关键词找到以下内容' : '没有精确匹配，以下最近资料供你继续查阅'}：${evidence}。展开下方依据查看已保存的正文。` : '先导入文本资料，或把学习笔记存入知识库，再开始检索。', citations };
}
...
function AssistantPanel({ knowledge, onConvert, onSave, compact = true }) {
  ...
  return <Localize><GlassSurface className="assistant-panel">
...
function SpatialDock({ page, setPage, openUpload }) {
  ...
  return <Localize><nav className="spatial-dock glass-surface-static" aria-label="空间导航">
...
function Dashboard({ knowledge, setPage, startFocus, openReading }) {
  ...
  return <Localize><section className="hub-home strict-reference" aria-label="My Knowledge Hub">
...
function LibraryPage({ knowledge, setKnowledge, openUpload, notify }) {
  ...
  return <Localize><div className="library-layout">
...
function AIPage({ knowledge, setKnowledge, actions, setActions, notify }) {
  ...
  const runQuery = (nextQuery = query) => {if (!nextQuery.trim()) return;setAnswer(makeAnswer(nextQuery, knowledge, scope));};
  const saveAnswer = () => {const id = `answer-${Date.now()}`;setKnowledge((current) => [{ id, title: `检索笔记 · ${query.slice(0, 40)}`, type: '检索笔记', fileType: 'md', space: '阅读与写作', size: '1 KB', updatedAt: new Date().toISOString(), status: 'ready', tags: ['检索笔记'], summary: answer.body.slice(0, 100), content: `问题：${query}\n${answer.body}\n来源：${answer.citations.map((item) => item.title).join('、')}`, source: '本地检索 · 保存的结果', favorite: false }, ...current]);notify('已实际保存到知识库');};
  ...
  return <Localize><div className="ai-workspace">
...
export function App() {
  ...
  return <Localize><div className="app-shell studio-shell"><img className="ambient-background" src={`${import.meta.env.BASE_URL}juzizhou-study.png`} fetchPriority="high" decoding="async" alt="" aria-hidden="true" /><div className="ambient-scrim" /><LearningStudio page={page} setPage={setPage} knowledge={knowledge} setKnowledge={setKnowledge} actions={actions} setActions={setActions} openUpload={() => setUploadOpen(true)}>{page === "library" && <LibraryPage knowledge={knowledge} setKnowledge={setKnowledge} openUpload={() => setUploadOpen(true)} notify={notify} />} {page === "ai" && <AIPage knowledge={knowledge} setKnowledge={setKnowledge} actions={actions} setActions={setActions} notify={notify} />} {page === "expression" && <ExpressionTraining />}{page === "strategy" && <StrategyPage />}</LearningStudio><AnimatePresence>{uploadOpen && <UploadSheet onClose={() => setUploadOpen(false)} onImport={(items) => {setKnowledge((current) => [...items, ...current]);setPage("library");}} notify={notify} />}</AnimatePresence><AnimatePresence>{toast && <Toast message={toast} />}</AnimatePresence></div></Localize>;
}
