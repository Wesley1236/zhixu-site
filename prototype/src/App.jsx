...
function Dashboard({ knowledge, setPage, startFocus, openReading, readingState }) {
  const activeKnowledge = knowledge.filter((item) => !item.deleted);
  const [hubSearch,setHubSearch] = useState("");
  const spring = { type:"spring",stiffness:340,damping:30 };
  const sideNav = [[House,"总览","dashboard"],[Books,"知识库","library"],[Sparkle,"AI 智识","ai"],[Target,"目标与项目","goals"],[BookOpenText,"读书笔记","reading"],[BookmarkSimple,"我的收藏","library"],[ArrowCounterClockwise,"成长复盘","review"],[SlidersHorizontal,"界面设置","strategy"]];
  const modules = [[Books,"知识库","library"],[Sparkle,"AI 助手","ai"],[Target,"目标项目","goals"],[NotePencil,"读书笔记","reading"],[UploadSimple,"分类上传","upload"],[ArrowCounterClockwise,"成长复盘","review"]];
  const go = (target) => target === "reading" ? openReading() : target === "upload" ? document.querySelector(".dock-capture")?.click() : setPage(target);
  return <section className="hub-home" aria-label="个人成长知识工作台">
    <aside className="hub-sidebar light-glass"><button className="hub-brand" onClick={()=>setPage("dashboard")}><span>知</span><strong>知序</strong><small>Knowledge Hub</small></button><nav>{sideNav.map(([Icon,label,target])=><motion.button key={label} className={target==="dashboard"?"active":""} onClick={()=>go(target)} whileTap={{scale:.96}} transition={spring}><Icon size={19}/><span>{label}</span><CaretRight size={14}/></motion.button>)}</nav><button className="hub-profile" onClick={()=>setPage("strategy")}><span className="avatar">W</span><span><strong>Wesley</strong><small>持续成长中</small></span><CaretRight size={15}/></button></aside>
    <main className="hub-main">
      <header className="hub-top"><form className="hub-search light-glass" onSubmit={(e)=>{e.preventDefault();setPage(hubSearch.trim()?"ai":"library")}}><MagnifyingGlass size={21}/><input value={hubSearch} onChange={(e)=>setHubSearch(e.target.value)} placeholder="搜索知识、任务或直接提问…"/><button aria-label="提交搜索"><ArrowRight size={17}/></button></form><motion.button className="hub-icon-button light-glass" onClick={()=>setPage("review")} whileTap={{scale:.94}}><Sparkle size={19}/><i/></motion.button><motion.button className="hub-icon-button light-glass" onClick={()=>setPage("goals")} whileTap={{scale:.94}}><CalendarDots size={19}/></motion.button><button className="hub-time light-glass" onClick={()=>setPage("review")}><strong>{new Intl.DateTimeFormat("zh-CN",{hour:"2-digit",minute:"2-digit",hour12:false}).format(new Date())}</strong><small>{new Intl.DateTimeFormat("zh-CN",{month:"long",day:"numeric"}).format(new Date())}</small></button></header>
      <div className="hub-greeting"><span><Sparkle size={32} weight="duotone"/></span><div><h1>早上好，Wesley</h1><p>保持好奇，持续成长。</p></div><blockquote>“预测未来最好的方式，是创造它。”</blockquote></div>
      <div className="hub-workspace">
        <div className="hub-left-stack"><motion.button className="focus-glass light-glass" onClick={startFocus} whileHover={{y:-4}} whileTap={{scale:.98}} transition={spring}><span className="hub-card-title">今日焦点 <CaretRight size={15}/></span><div className="focus-content"><span className="focus-ring"><b>68%</b><small>进度</small></span><ul><li><CheckCircle size={15}/>检索逻辑</li><li><CheckCircle size={15}/>引用回链</li><li><CircleNotch size={15}/>关键测试</li></ul></div><strong>完成个人知识检索模块</strong><small>预计专注 90 分钟</small></motion.button><div className="quick-glass light-glass"><span className="hub-card-title">快速入口</span><div>{[[ShieldCheck,"标准"],[BookOpenText,"精读"],[FileText,"模板"],[BookmarkSimple,"收藏"]].map(([Icon,label],index)=><motion.button key={label} onClick={()=>go(index===1?"reading":"library")} whileHover={{y:-3}} whileTap={{scale:.94}} transition={spring}><Icon size={21}/><small>{label}</small></motion.button>)}</div></div></div>
        <div className="hub-center"><div className="hub-core light-glass"><span><Brain size={45} weight="duotone"/></span><strong>成长中枢</strong><small>知识连接行动，行动沉淀成长</small></div>{modules.map(([Icon,label,target],index)=><motion.button key={label} className={`hub-module module-${index} light-glass`} onClick={()=>go(target)} whileHover={{y:-6,scale:1.025}} whileTap={{scale:.95}} transition={spring}><Icon size={27} weight="duotone"/><strong>{label}</strong><small>{target==="library"?`${activeKnowledge.length} 份知识`:target==="reading"?(readingState.completed?"今日已完成":"今日待精读"):"立即进入"}</small></motion.button>)}</div>
        <div className="hub-right-stack"><motion.button className="stats-glass light-glass" onClick={()=>setPage("library")} whileHover={{y:-3}} whileTap={{scale:.98}} transition={spring}><span className="hub-card-title">知识统计 <CaretRight size={15}/></span><div><span><b>{activeKnowledge.length}</b><small>知识</small></span><span><b>{new Set(activeKnowledge.map(i=>i.space)).size}</b><small>分类</small></span><span><b>{activeKnowledge.filter(i=>i.favorite).length}</b><small>收藏</small></span></div><span className="stats-bars">{[35,52,46,70,61,82,78].map((v,i)=><i key={i} style={{height:`${v}%`}}/>)}</span></motion.button><div className="activity-glass light-glass"><span className="hub-card-title">最近活动 <button onClick={()=>setPage("library")}>查看全部</button></span>{activeKnowledge.slice(0,4).map((item)=><motion.button key={item.id} onClick={()=>setPage("library")} whileHover={{x:3}} whileTap={{scale:.98}} transition={spring}><FileTypeIcon item={item} size={18}/><span><strong>{item.title}</strong><small>{item.space}</small></span><CaretRight size={14}/></motion.button>)}</div></div>
      </div>
      <form className="hub-ask light-glass" onSubmit={(e)=>{e.preventDefault();setPage("ai")}}><Sparkle size={20}/><input value={hubSearch} onChange={(e)=>setHubSearch(e.target.value)} placeholder="向你的知识库提问…"/><button aria-label="向知识库提问"><ArrowRight size={19}/></button></form><div className="hub-prompts">{["总结一份文档","比较两个观点","寻找关键洞见","生成行动清单"].map((text)=><button key={text} onClick={()=>{setHubSearch(text);setPage("ai")}}>{text}</button>)}</div>
    </main>
  </section>;
}
