...
function Dashboard({ knowledge, setPage, startFocus, openReading, readingState }) {
  ...
  return <section className="command-home" aria-label="今日成长桌面">
    <div className="command-signature"><strong>知序 <small>4.0</small></strong><span>个人成长知识 OS</span></div>
    <header className="command-greeting"><span>{new Intl.DateTimeFormat("zh-CN",{year:"numeric",month:"2-digit",day:"2-digit",weekday:"long"}).format(new Date())}</span><h1>晚上好，Wesley</h1></header>
    <div className="command-orbit">
      <motion.button className="command-module action-module glass-surface-static" onClick={startFocus} whileHover={{ y:-5,scale:1.01 }} whileTap={{ scale:.97 }} transition={{ type:"spring",stiffness:330,damping:30 }}><span className="module-icon"><Target size={25} weight="duotone"/></span><h2>今日行动</h2><span className="module-kicker">今日目标</span><strong>完成个人知识检索模块</strong><div className="action-time"><Clock size={16}/>90 分钟</div><span className="module-kicker">下一步行动</span><p>补齐引用回链和 3 个关键测试</p><div className="action-proof"><span><b>68%</b><small>已完成</small></span><ul><li><Check size={13}/>完成检索逻辑</li><li><Check size={13}/>建立引用回链</li><li><CircleNotch size={13}/>补齐关键测试</li></ul></div><em>开始推进 <ArrowRight size={17}/></em></motion.button>
      <motion.button layoutId="reading-module" className="command-module reading-module glass-surface-static" onClick={openReading} whileHover={{ y:-4,scale:1.012 }} whileTap={{ scale:.975 }} transition={{ type:"spring",stiffness:290,damping:30 }}><span className="module-icon"><BookOpenText size={29} weight="duotone"/></span><h2>今日精读</h2><div className="reading-book"><span>正在精读</span><strong>《卓有成效的管理者》</strong><small>第 2 章：掌握自己的时间</small><em><Clock size={16}/>预计 25 分钟</em></div><div className="reading-focus"><span>今日聚焦</span><p>先记录时间，再消除浪费，最后合并可支配时间</p></div><span className="module-kicker centered">精读笔记 · 三步引导</span><div className="reading-steps">{[[NotePencil,"摘录一句"],[Brain,"解释为什么"],[PaperPlaneTilt,"写出明日行动"]].map(([Icon,label],index)=><span key={label} className={readingState.steps?.[index]?"done":""}><i>{readingState.steps?.[index]?<Check size={14}/>:<Icon size={18}/>}</i>{label}</span>)}</div><span className="reading-cta">进入精读模式 <ArrowRight size={18}/></span></motion.button>
      <motion.button className="command-module knowledge-module glass-surface-static" onClick={() => setPage("ai")} whileHover={{ y:-5,scale:1.01 }} whileTap={{ scale:.97 }} transition={{ type:"spring",stiffness:330,damping:30 }}><span className="module-icon"><Brain size={26} weight="duotone"/></span><h2>知识与 AI</h2><span className="ai-subtitle">基于 {activeKnowledge.length} 份本地知识，为你赋能</span><div className="ai-capabilities"><span><i><ChatCircle size={19}/></i><b>知识问答与洞见</b></span><span><i><PuzzlePiece size={19}/></i><b>个性化学习路径</b></span><span><i><NotePencil size={19}/></i><b>写作与思考伙伴</b></span></div><em>进入知识助手 <ArrowRight size={17}/></em></motion.button>
    </div>
    <button className="reflection-strip glass-surface-static" onClick={openReading}><span><Sparkle size={21}/><strong>完成后分享读书想法与笔记</strong><small>我今天真正改变的一个认知是什么？</small></span><em>{readingState.completed ? "继续完善笔记" : "记录并分享"}<ArrowRight size={18}/></em></button>
  </section>;
}
