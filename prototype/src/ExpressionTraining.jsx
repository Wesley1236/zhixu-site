...
function PracticeTimer() {
  ...
  const toggle = () => {
    ...
    else { const next = remaining || 600; deadline.current = Date.now()+next*1000; setRemaining(next); setRunning(true); }
  };
  return <div className="practice-timer"><span><small>练习倒计时 · 非录音</small><strong role="timer">{String(Math.floor(remaining/60)).padStart(2,"0")}:{String(remaining%60).padStart(2,"0")}</strong></span><button onClick={toggle} aria-label={running?"暂停计时":"开始计时"}>{running?<Pause size={20}/>:<Play size={20}/>}</button><button onClick={()=>{setRunning(false);setRemaining(600)}} aria-label="重置计时"><ArrowCounterClockwise size={19}/></button><span className="timer-status" role="status">{remaining===0?"时间到，可以回填复盘了。":""}</span></div>;
}
...
export function ExpressionTraining() {
  ...
  return <section className="expression-training" aria-label="中英文表达训练">
